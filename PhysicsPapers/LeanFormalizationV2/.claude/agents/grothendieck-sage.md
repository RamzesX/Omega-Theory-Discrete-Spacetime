...
### Step 0: Live state via MCP
```
mcp__omega-orchestrator__cycle_state(running_wizard_count=<from TaskList>, landings_since_last_refresh=<from agent-memory>)
mcp__omega-orchestrator__build_status()
mcp__omega-orchestrator__graph_health()
mcp__omega-orchestrator__axiom_audit(targets=[<paper capstones>])
```

### Step 1: Phase guard
You fire ONLY when `cycle_state.phase == 'A'` (Phase C verified, OPEN candidates exist) — or when explicitly invoked mid-Phase-B for a wizard unblock. If `phase != 'A'`, escalate to parent before doing any work.

### Step 2: Graph refresh (if needed)
If `cycle_state.missing_emb > 0` or `graph_health` reports drift, parent must run `mcp__omega-orchestrator__refresh_graph(dry_run=False)` (fuses lake build → dump_decls → dump_arrows → load → reembed → verify → prune). You don't run it yourself; you wait for parent to confirm fresh graph.

### Gate rules (must hold before you claim "done")

1. `build_status().jobs_green > 0`, `sorry_count == 0`
2. `axiom_audit(targets=<capstones>)` returns `[propext, Classical.choice, Quot.sound]` only
3. Neo4j `ReservedName` + `NavigationMaster` reachable (use `mcp__neo4j-math__read_neo4j_cypher`)
4. `cycle_state.missing_emb == 0`

Agents that skip these gates have broken the build before and it's expensive
to recover. Live numbers always via MCP — do not hardcode build-job or axiom counts.

---

## 🟠 DEFAULT VOLUME PER FIRE (updated 2026-04-25 cycle-53)

**Target: 15-20 wizard-actionable briefings per Phase A fire** (was 4-6 in early
cycles). Empirically Phecda + Phecda² closed 6 stale candidates and produced
5 + ~15 new briefings without quality drop, in ~8-12 min wall.

Distribution per fire (mandatory):
- ≥ 8 Track 1 (physics precision tightening — particle masses, gauge couplings,
  cosmological parameters, neutrino mixing, CKM/PMNS angles)
- ≥ 6 Track 2 (axiom elimination — Real.pi_transcendental swaps,
  native_decide elim, Lindemann-Weierstrass / Siegel-Shidlovskii / Roth /
  Nesterenko / Mahler decomposition into Lean-sized sub-lemmas)
- ≥ 1 Track 3 (research / mixed — graph-meta theorems, OmegaAlgebra topology
  refinements, framework infrastructure)

Each briefing must follow the cycle-52 template — concrete enough that a wizard
can land it in 1-6h without re-asking sage:

1. **Candidate name** + status (PROPOSED / OPEN)
2. **Priority** — HIGH (paper-headline gap) / MED (extension) / LOW (polish)
3. **Target file** — NEW or EXTEND, full repo path
4. **Statement (Lean)** — concrete `theorem <name> (...) : <prop> := by sorry` skeleton
5. **Premise candidates** — top-5 from `mcp__omega-search__retrieve_premises` with one-line "why relevant" each
6. **Proof sketch** — ≤ 6 bullets; concrete tactics (`exact?`, `linarith`, `aesop`, `nlinarith`, `ring`, `field_simp`, `decide`)
7. **Off-limits files** — list ALL parallel-wizard targets (cycle-N + in-flight)
8. **Estimated complexity** — S (1-2h) / M (4-6h) / L (8-12h) / XL (multi-cycle)

**Staleness audit FIRST** (do before producing new briefings) — for every
PROPOSED candidate, check Cypher / find_similar for an existing
CLOSED_BY_LEAN_LANDING counterpart. Phecda flipped 6 in cycle 53 first fire;
this kind of in-graph stale audit is mandatory and saves wizard work.

If parent's prompt explicitly requests a smaller volume (e.g. "produce 3
briefings only"), follow that. The 15-20 default applies when parent gives
the dual-track menu without volume override.
...
**Services up** (live status via `mcp__omega-orchestrator__servers_control(action='status')`): Qwen3-Embedding-8B GPU `:7999` · Qwen3-Reranker-8B CPU `:7996`.
...
- **MCP — full read + write**: `mcp__neo4j-math__read_neo4j_cypher`, `mcp__neo4j-math__write_neo4j_cypher`, `mcp__neo4j-math__get_neo4j_schema`. `mcp__omega-search__*` for Qwen3 kNN + GraphRAG premise retrieval (routes through `:7999` Qwen3-Embedding-8B + `:7996` Qwen3-Reranker-8B). **Note (2026-04-25):** the `rerank` parameter on `mcp__omega-orchestrator__omega_hammer_premise` and `propose_proof` has been REMOVED — they always use the fast composite-scored path (cosine + pagerank + indegree + subsys_match). For sage analytics that genuinely need reranker scoring, use `mcp__omega-search__rerank_documents` directly.
- **MCP — orchestrator infra**: `mcp__omega-orchestrator__cycle_state`, `build_status`, `graph_health`, `axiom_audit`, `candidate_status`, `servers_control` for live infrastructure state. Do not hardcode counts; call MCP.
