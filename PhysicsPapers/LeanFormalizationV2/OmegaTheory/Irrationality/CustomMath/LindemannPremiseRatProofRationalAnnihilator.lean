...
/-! ## Phase 3 — Bridge to session 6 `exp_polynomial_approx_at_iPi`
    (integer-polynomial extraction PROVEN in session 9) -/

/-- **Wave T4b session 7 — analytical bound at i·π via integer extraction**.

    Composes session 7's rational-annihilator with session 6's
    `exp_polynomial_approx_at_iPi`: given a ℤ-poly `f` whose roots
    in ℂ include `i*π`, the analytical bound holds. -/
...
theorem lindemann_premise_rat_proof_w_t4b_session_7_headline :
    ((Complex.I * (Real.pi : ℂ)) ∈ nonZeroDivisors ℂ)
      ...
      ∧ (∀ (f : Polynomial ℤ) (_hf : Polynomial.eval 0 f ≠ 0)
          (_h_iPi_root : (Complex.I * (Real.pi : ℂ)) ∈ f.aroots ℂ),
          ∃ c : ℝ, ∀ p > (Polynomial.eval 0 f).natAbs, p.Prime →
            ∃ n : ℤ, ¬ (↑p ∣ n) ∧ ∃ gp : Polynomial ℤ,
              gp.natDegree ≤ p * f.natDegree - 1 ∧
              ‖n • Complex.exp (Complex.I * (Real.pi : ℂ))
