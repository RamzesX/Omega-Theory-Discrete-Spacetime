...
def targetKeep (includeMathlib : Bool) (n : Name) : Bool :=
  let s := n.toString
  s.startsWith "OmegaTheory." ||
    (includeMathlib && s.startsWith "Mathlib.")
...
def filtDedup (includeMathlib : Bool) (xs : Array Name) : Array String :=
  xs.filter (targetKeep includeMathlib)
...
def renderConstantRecord
  ...
  -- --- Filter + dedup ---
  let appliesArr := filtDedup includeMathlib valueConsts
  let assumesArr := filtDedup includeMathlib assumes
  let unfoldsArr := filtDedup includeMathlib typeConsts
  let reducesArr := filtDedup includeMathlib reducesTo
  let extendsStr := filtDedup includeMathlib extendsArr
...
def renderModuleRecord
  ...
  if !inNamespace includeMathlib moduleName then return none
  let imports : Array String :=
    md.imports.filterMap fun imp =>
      let n := imp.module
      if targetKeep includeMathlib n then some n.toString else none
  let fields : List String :=
...
unsafe def main (args : List String) : IO UInt32 := do
  ...
  for (moduleName, md) in env.header.moduleNames.zip env.header.moduleData do
    if let some recStr := renderModuleRecord env includeMathlib moduleName md then
      write recStr
      nModules := nModules + 1
      if md.imports.any (fun i => targetKeep includeMathlib i.module) then
